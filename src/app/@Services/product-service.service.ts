  ...
  search(req: SearchProductReq): Observable<GetProductDataRes> {
    return this.http.post<any>(`${this.BASE}/search`, req).pipe(
      map(res => res.products.map((p: any) => ({
        ...
        price: p.price,
        time: p.shelfDate,
        imgUrl: p.imgPath && p.imgPath.length > 0 ? p.imgPath[0] : 'https://www.townlinepaint.com/cdn/shop/products/B6B4B2.png?v=1646778952&width=1200/default.jpg',
        location: p.location,
        user: {
          userName: '李吉娃娃', // 或者先給預設名稱
